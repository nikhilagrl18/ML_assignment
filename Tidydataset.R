...
completeNames <- c("subject", "activity", as.character(features$V2))
meanStdColumns <- grep("subject|activity|[Mm]ean|std", completeNames, value = FALSE)
reducedsubset <- completeset[ ,meanStdColumns]
...
reducedNames <- completeNames[meanStdColumns]
